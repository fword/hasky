...
def get_weights(name, shape, minval=-0.08, maxval=0.08):
  return tf.get_variable(name, shape, initializer=tf.random_uniform_initializer(minval, maxval))

def get_weights_truncated(name, shape, stddev=1.0):
  return tf.get_variable(name, shape, initializer=tf.truncated_normal_initializer(stddev=stddev))

def get_weights_random(name, shape, stddev=1.0):
  return tf.get_variable(name, shape, initializer=tf.random_normal_initializer(stddev=stddev))
 

def get_weights_uniform(name, shape, minval=0, maxval=None):
  return tf.get_variable(name, shape, initializer=tf.random_uniform_initializer(minval, maxval))
